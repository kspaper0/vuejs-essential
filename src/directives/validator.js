function validate(el, modifiers, bindingValue) {
  ...
  const { title = 'The field', error } = bindingValue
  let defaultError = ''

  if (modifiers.required && value === '') {
    defaultError = `${title} cannot be null`
    ...
    if (targetValue !== value) {
      defaultError = `${title} is not match of password`
    }
  } else if (bindingValue.regex) {
    try {
      if (!bindingValue.regex.test(value)) {
        defaultError = `${title} format is incorrect`
      }
    } catch (e) {}
  }
